...
Zahvaljujem se mentorju Alešu Volčiniju za pomoč, usmerjanje in koristne pripombe pri izdelavi seminarske naloge ter pri razvoju aplikacije MediForm.

Posebej se zahvaljujem Heleni Božič Janežič s Srednje zdravstvene šole Ljubljana za sodelovanje, pojasnila o obstoječem načinu dela, posredovana gradiva in sprotne povratne informacije. Zahvaljujem se tudi dijakom, ki so aplikacijo preizkusili in s svojimi predlogi pomagali izboljšati uporabniško izkušnjo.

\newpage

\section*{Povzetek}

V tem strokovnem poročilu predstavljam razvoj spletne aplikacije MediForm, ki sem jo izdelal za Srednjo zdravstveno šolo Ljubljana. Aplikacija je namenjena dijakom zdravstvene nege za sprotno in strukturirano izpolnjevanje poročil o praktičnem pouku. Za digitalno rešitev sem se odločil zato, ker so bila papirna poročila zamudna, neenotna in pogosto izpolnjena šele tik pred rokom oddaje.

V poročilu opisujem analizo potreb naročnika, potek dogovarjanja in komunikacije s stranko, izbiro tehnologij ter tehnično izvedbo ključnih funkcionalnosti. Ker je končna rešitev zasnovana kot frontend-only aplikacija, podatke shranjujem v piškotke in lokalno shrambo brskalnika. Tak pristop sem izbral zato, ker je aplikacija namenjena preprosti uporabi brez dodatne strežniške infrastrukture, šola pa ima tako večji nadzor nad informacijami in lažje upravlja z objavljeno različico.

Pri vizualni podobi sem logotip najprej ustvaril s pomočjo ChatGPT, nato pa sem v Canvi dodal še napis MediForm in prilagodil končni videz.

Ugotovil sem, da je dobra in sprotna komunikacija s stranko zelo pomembna za uspešen razvoj ter da aplikacija uporabnikom dejansko olajša delo.
...
V današnjem času se šole vse pogosteje srečujejo s potrebo po digitalizaciji administrativnih postopkov. Tudi v zdravstvenem izobraževanju se pojavlja težava, da dijaki poročila o praktičnem pouku pogosto izpolnjujejo prepozno, površno ali neenotno. To zmanjšuje preglednost njihovega dela in otežuje sprotno spremljanje opravljenih aktivnosti.

Zaradi tega konkretnega problema sem se odločil razviti spletno aplikacijo MediForm. Z njo sem želel dijakom omogočiti, da poročila izpolnjujejo sproti, z vnaprej pripravljenimi možnostmi izbire in z manj ročnega tipkanja. Hkrati sem želel izdelati rešitev, ki bi bila dovolj pregledna, varna in preprosta za uporabo v šolskem okolju.
...
\begin{itemize}
    ...
    \item opisati ključne dele kode in razloge za takšno zasnovo,
    \item prikazati, kako je potekalo delo in komunikacija s stranko,
    \item predstaviti rezultate analize uporabe aplikacije.
\end{itemize}

\section{Teoretični uvod}

Digitalna transformacija v izobraževanju pomeni uvajanje digitalnih tehnologij v učne in administrativne procese. Pri strokovnem in poklicnem izobraževanju je ta prehod še posebej pomemben, saj omogoča boljšo preglednost, hitrejše delo in manj napak pri dokumentiranju. Evropska komisija poudarja, da digitalna orodja izboljšujejo dostopnost informacij in omogočajo učinkovitejše upravljanje izobraževalnih procesov (European Commission, 2020).

Pri digitalnih obrazcih je pomembno, da so postopki vnosa podatkov enostavni in strukturirani. Selwyn (2016) izpostavlja, da digitalni sistemi zmanjšujejo potrebo po ponavljajočem ročnem zapisovanju in omogočajo bolj dosledno obdelavo podatkov. Tudi razvoj oblačnih rešitev, kot je Firebase, prispeva k večji dostopnosti in prilagodljivosti aplikacij, saj podatki niso vezani le na eno napravo ali eno lokacijo (Firebase, 2026).

V MediForm sem zato upošteval načela digitalne preglednosti, varnega shranjevanja podatkov in sprotnega beleženja. Takšen pristop je bil primeren za okolje zdravstvene nege, kjer je pomembno, da dijaki poročila izpolnjujejo redno in v čim bolj enotni obliki.

Pri razvoju spletne aplikacije sem se opiral tudi na osnovna načela spletnih tehnologij. HTML sem uporabil kot osnovno strukturo obrazcev, CSS oziroma Tailwind CSS za oblikovanje in preglednost, JavaScript pa za logiko uporabniške interakcije. React mi je omogočil, da sem posamezne dele vmesnika razdelil v komponente in jih lažje ponovno uporabil, TypeScript pa mi je pomagal pri natančnejšem razvoju, saj sem že med pisanjem hitreje opazil morebitne napake v tipih.

Za lokalni razvoj in hitrejše testiranje sem uporabil Vite, ker sem lahko spremembe vmesnika hitro preverjal brez dolgotrajnega zaganjanja. To je bilo pomembno predvsem pri delu z več vnosnimi polji, tabelami in izbirnimi seznami, kjer je bilo treba večkrat preveriti, kako se elementi obnašajo na različnih zaslonih. Takšna izbira tehnologij je bila skladna z mojim ciljem, da aplikacijo razvijem pregledno, hitro in dovolj preprosto za šolsko uporabo.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.28\textwidth]{slike/zdravstvena_sola_jubljana_logo.png}
    \caption{Logotip Srednje zdravstvene šole Ljubljana, naročnika spletne aplikacije}
\end{figure}
...
Proces dogovarjanja sem razdelil na več faz. Najprej sem prejel papirne obrazce in skupaj z naročnico pregledal, kateri deli so zares potrebni in katere bi bilo mogoče poenostaviti. Na podlagi tega sem zasnoval prvo različico aplikacije.

Ko so dijaki začeli aplikacijo uporabljati, sem dobil prve povratne informacije. Ugotovil sem, da so se pri prijavi pojavile določene težave, zato sem popravil način shranjevanja stanja in izboljšal stabilnost delovanja. Kasneje je naročnica predlagala, da bi bilo bolje, če bi dijaki večino podatkov le označevali z izborom možnosti, namesto da jih ročno vnašajo.

V nadaljevanju sem aplikacijo dopolnjeval glede na pripombe iz prakse. Naročnica je posebej poudarila, da mora biti rešitev primerna za uporabo na telefonu, omogočati sprotno izpolnjevanje in poenostaviti nekatere dele obrazca. Na tej osnovi sem odstranil odvečne elemente in izboljšal uporabniški vmesnik.
...
\begin{figure}[h]
    \centering
    \includegraphics[width=0.22\textwidth]{slike/react_logo.png}
    \hspace{1.5cm}
    \includegraphics[width=0.35\textwidth]{slike/tailwindlogo.png}
    \caption{Logotipa knjižnic React in Tailwind CSS}
\end{figure}

Za ta izbor sem se odločil, ker omogoča hitro razvojno delo, dobro preglednost kode in enostavno nadgradnjo aplikacije. V produkcijski različici sem se odpovedal strežniškemu delu, saj gre za frontend-only rešitev, pri kateri želim imeti podatke pod nadzorom znotraj same spletne strani in brskalnika.

Pri primerjavi z lanskimi rešitvami sem posebej pazil, da so tehnološka pojasnila krajša, vendar bolj povezana z dejanskim delovanjem aplikacije. Namesto splošnega opisa sem se osredotočil na to, kako React, TypeScript in piškotki konkretno pomagajo pri izpolnjevanju obrazca, prikazu podatkov in preprostem objavljanju spletne strani.

\subsubsection{Primer importov in namen knjižnic}

Spodaj je primer dejanskih \texttt{import} stavkov iz aplikacije, kjer je jasno vidno, katere knjižnice in komponente uporabljam:
...
Razlaga: knjižnica \texttt{react} omogoča gradnjo vmesnika s komponentami in stanje obrazca (\texttt{useState}, \texttt{useEffect}). Knjižnica \texttt{react-router-dom} skrbi za prehode med stranmi, na primer med začetnim zaslonom, obrazcem in pravnimi stranmi. Komponenta \texttt{CookieConsent} upravlja soglasje za piškotke, komponenta \texttt{Checklist} pa je osrednji del za izpolnjevanje poročila. Knjižnici \texttt{pdf-lib} in \texttt{@pdf-lib/fontkit} uporabljam za generiranje PDF poročila in pravilno delo s pisavami pri izvozu dokumenta.
...
Namesto strežniške prijave sem uporabil preprost pristop z brskalniškimi piškotki. Ti so primerni za krajše shranjevanje pomembnih nastavitev, saj jih lahko aplikacija prebere ob naslednjem obisku strani.
...
Kontrola verzij je v projektu potekala z orodjem Git po postopnem in iterativnem pristopu razvoja. V repozitoriju je zabeleženih 73 commitov v obdobju od 29. 10. 2024 do 20. 1. 2026. Razvoj je potekal predvsem na veji \texttt{main}, občasno pa tudi na razvojni veji \texttt{dev}, kar je omogočilo varno pripravo sprememb in njihovo poznejše združevanje. V zgodovini so vidni tudi merge commiti in vsaj en pull request, zato je bil uporabljen standarden sodelovalni potek dela (branching, pregled sprememb in združevanje).

Iz zgodovine commitov je razviden prehod od začetnih manjših popravkov k bolj strukturiranemu razvoju funkcionalnosti. Med pomembnejše faze sodijo implementacija izvoza PDF, izboljšave uporabniškega vmesnika, uvedba GDPR elementov (piškotki, pogoji uporabe, politika zasebnosti), optimizacije postavitve komponent in kasnejši refaktor arhitekture v smeri local-first pristopa. Del starejših commitov vsebuje bolj neformalne opise, v kasnejšem obdobju pa so sporočila bolj standardizirana (na primer \texttt{feat}, \texttt{fix}, \texttt{refactor}, \texttt{chore}), kar izboljša sledljivost sprememb in olajša vzdrževanje kode.

Tak način kontrole verzij je omogočil:
\begin{itemize}
    \item jasno sledljivost razvoja skozi čas,
    \item varno uvajanje novih funkcionalnosti brez izgube prejšnjih stanj,
    \item lažje odpravljanje napak in vračanje na stabilne različice,
    \item boljšo organizacijo timskega dela prek vej in merge postopkov.
\end{itemize}
...
Razlaga: Prvi ukaz pripravi datoteke za shranjevanje, drugi pa ustvari novo verzijo z opisom spremembe. Tak način beleženja sprememb je zadostoval za jasno sledljivost razvoja in enostavno vračanje na prejšnja stabilna stanja.

\section{Deployment (objava aplikacije)}

Deployment sem izvedel kot objavo statične frontend aplikacije na platformi Vercel. Ker MediForm ne uporablja ločenega strežniškega dela, je bil postopek enostaven: priprava produkcijskega \texttt{build} paketa in objava na domeni \url{https://mediform.cadez.eu/}. V nadaljevanju je verjeten prehod na lokalno gostovanje, saj bi to šoli omogočilo popoln nadzor nad aplikacijo, podatki in načinom posodabljanja.
...
Tak pristop je bil primeren za šolsko okolje, ker je objava hitra, stroški so nizki, vzdrževanje pa preprostejše kot pri aplikaciji z dodatnim strežnikom.
...
Ugotovil sem, da je bila najpomembnejša dobra komunikacija s stranko. Sproti sem preverjal, kaj naročnica dejansko potrebuje, in na podlagi njenih pripomb popravljal aplikacijo. To se je izkazalo za pravilno odločitev, saj so bile funkcionalnosti bolj usmerjene v resnične potrebe uporabnikov.

Pri tehnični izvedbi sem se srečal z več izzivi, predvsem pri organizaciji obrazcev, samodejnem shranjevanju podatkov in pripravi izvoza v PDF. Vsak problem sem reševal postopno in sproti preverjal skupaj z naročnico, zato je bila končna rešitev stabilna in uporabna v praksi.

Na koncu sem ugotovil, da MediForm uporabnikom olajša delo, zmanjšuje količino ročnega pisanja in omogoča bolj sprotno izpolnjevanje dokumentacije. Zato menim, da je digitalizacija tovrstnih postopkov smiselna in koristna rešitev za izobraževalno okolje.
...
Izjavljam, da je strokovno poročilo \textit{Spletna aplikacija MediForm za digitalno poročanje o zdravstveni negi} v celoti moje avtorsko delo, ki sem ga izdelal samostojno s pomočjo navedene literature in pod vodstvom mentorja.
